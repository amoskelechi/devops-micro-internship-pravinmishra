...
![Screenshot 1 - Output of `git init` inside `CodeTrack` showing "Initialized empty Git repository](screenshots/assignment-codetrack-task1-screenshot1.png)

---

#### Screenshot 2 — Output of `ls -a` showing the `.git` folder

![Screenshot 2 - Output of `ls -a` showing the `.git` folder](screenshots/assignment-codetrack-task1-screenshot2.png)
...
What is the .git folder, and why does it matter? — The .git folder is where Git stores everything about the repository's history: commits, branches, configuration, and the object database that tracks every version of every tracked file. It's what actually turns a normal folder into a Git repository — deleting it would remove all version history while leaving your working files untouched.
...
![Screenshot 3 — Output of `git config --local --list` showing your `user.name` and `user.email`](screenshots/assignment-codetrack-task2-screenshot1.png)
...
![Screenshot 4 — Output of `git config --global --list` showing your `user.name` and `user.email`](screenshots/assignment-codetrack-task3-screenshot1.png)
